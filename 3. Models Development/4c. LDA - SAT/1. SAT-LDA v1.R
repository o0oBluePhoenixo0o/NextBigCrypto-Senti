# Building Sentiment Analysis with LDA - Trained model 24.05.2018
...
packages <- c("readr", #read data
              ...
              "wordcloud","tidyquant","tidyr","topicmodels",
...
# LDA model with data until 15.04.2018
...
df_classifications <- Reassign(df_lda)
...
# Merge to get final new BTC.clean
...
# Per / topic
BTC.senti.trained.tp <- BTC.senti %>% 
  dplyr::select(date,sentiment.trained,topic) %>%
  group_by(time = floor_date(date, paste0(time.slot,' hour')),
           sentiment.trained,
           topic) %>%
  summarize(count.tp = n()) %>%
  group_by(time) %>%
  mutate(countT = sum(count.tp)) %>%
  group_by(sentiment.trained) %>%
  mutate(per.tp = round(100* count.tp/countT,2))

# Until HERE
# save.image('~/GitHub/NextBigCrypto-Senti/3. Models Development/4. SAT-LDA - Sentiment (Trained)/BTC_SAT-LDA_2505.RData')
# load('~/GitHub/NextBigCrypto-Senti/3. Models Development/4. SAT-LDA - Sentiment (Trained)/BTC_SAT-LDA_2505.RData')

# Convert to each sentiment = column
BTC.senti.trained.tp <- reshape2::dcast(BTC.senti.trained.tp, time + countT + topic ~ sentiment.trained,
                                        value.var = 'per.tp')
