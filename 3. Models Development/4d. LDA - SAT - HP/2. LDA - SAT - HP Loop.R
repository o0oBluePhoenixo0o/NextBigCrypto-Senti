...
for (y in 1:length(time.set)){
  ...
  # Merge senti + LDA
  BTC.senti.LDA.df <- inner_join(BTC.senti.trained, BTC.LDA.df, by = 'time')
...
# Save final result
write.xlsx(final.result,'~/GitHub/NextBigCrypto-Senti/3. Models Development/0. LDA-SAT-HP_result.xlsx')

# Save model
save.image('~/GitHub/NextBigCrypto-Senti/Models/LDA-SAT-HP_LOOP_0806.RData')
