...
result$status_id <- BTC$status_id
...
result <- mutate(result, status_id, sentiment.packages = ifelse(result$score > 0, 1, 
  ...
  select(status_id, sentiment.packages)
...
BTC.senti.packages <- BTC.senti.final %>% 
  select(date,sentiment.packages) %>%
  group_by(date, sentiment.packages) %>%
  summarize(count =n())
...
BTC.senti.packages <- dcast(BTC.senti.packages, date + countT ~ sentiment.packages , 
                            value.var = 'per')
colnames(BTC.senti.packages) <- c('date','count.total','negative','neutral','positive')
